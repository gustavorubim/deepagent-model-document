...
def test_apply_rejects_table_templates(tmp_path: Path, table_template_path: Path) -> None:
    with pytest.raises(UnsupportedTemplateError):
        apply_draft_to_template(table_template_path, _build_draft(), tmp_path / "out.docx")
